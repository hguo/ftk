...
template <typename T>
__device__ __host__
inline void lerp_s1v4(const T V[2][4], const T mu[2], T v[4])
{
  v[0] = V[0][0] * mu[0] + V[1][0] * mu[1];
  v[1] = V[0][1] * mu[0] + V[1][1] * mu[1];
  v[2] = V[0][2] * mu[0] + V[1][2] * mu[1];
  v[3] = V[0][3] * mu[0] + V[1][3] * mu[1];
}
...
template <typename T>
__device__ __host__
inline void lerp_s2v3x3(const T J[3][3][3], const T mu[3], T j[3][3])
{
  for (int i = 0; i < 3; i ++)
    for (int k = 0; k < 3; k ++)
      j[i][k] = J[0][i][k] * mu[0] + J[1][i][k] * mu[1] + J[2][i][k] * mu[2];
}
