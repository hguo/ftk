...
struct mpas_particle_t {
  ...
  T scalar[FTK_CP_MAX_NUM_VARS-1]; // scalar attributes
  int hint_c = 0, hint_l = 0;

  __device__ __host__ T get_z() const { return R - R0; }
  __device__ __host__ void set_z(T z) { R = R0 + z; }

  __device__ __host__ void get_x(T x[3]) const {
    for (int i = 0; i < 3; i ++)
      x[i] = r[i] * R;
  }

  __device__ __host__ void set_x(const T x[3]) {
    R = vector_2norm<3>(x);
    for (int i = 0; i < 3; i ++)
      r[i] = x[i] / R;
  }

  __device__ __host__ void axis_rotate(const T axis[3], const T rad) { // axis rotation
  ...
  __device__ __host__ void step(const T dt)
  {
