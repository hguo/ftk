...
TEST_F(conv_test, 2D_conv_test) {
  hypermesh::ndarray<double> k({2,2}),d({3,3}),r;
...
TEST_F(conv_test, 2D_conv_padding_test) {
  hypermesh::ndarray<double> k({3,3}),d({3,3}),r;
...
TEST_F(conv_test, 2D_conv_gaussian_test) {
  hypermesh::ndarray<double> d({3,3}),r;
  std::vector<double> ans={3/4.,1.,6/4.,7/4.,},
                      res;
  d.from_vector({1,2,3,4,5,6,7,8,9,});
  r = hypermesh::conv2D_gaussian(d,1.,2,2);
  r.to_vector(res);
  EXPECT_EQ(res,ans);
}

TEST_F(conv_test, 3D_conv_test) {
  hypermesh::ndarray<double> d({3,3,3}),k({2,2,2}),r;
