...
void xft_execute(ctx_t *c, int scope, int current_timestep)
{
  ...
  hipLaunchKernelGGL(HIP_KERNEL_NAME(sweep_simplices<int, double>), dim3(gridSize), dim3(blockSize), 0, 0, 
    ...
    double *d_jacobian_out)
{
  ...
  hipLaunchKernelGGL(HIP_KERNEL_NAME(smooth_scalar_vector_jacobian<int, double>), dim3(gridSize), dim3(blockSize), 0, 0, 
      c->nphi, 
      c->m2n0, 
      c->d_m2coords,
      c->sigma,
