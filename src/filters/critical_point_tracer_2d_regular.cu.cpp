    ...
    const double *coords)
{
  ...
  double *dcoords = NULL;
  if (coords) {
    hipMalloc((void**)&dcoords, 2 * sizeof(double) * ext.n());
    hipMemcpy(dcoords, coords, 2 * sizeof(double) * ext.n(), hipMemcpyHostToDevice);
  }
  ...
  if (Vn) {
    hipMalloc((void**)&dVn, 2 * sizeof(double) * ext.n());
    hipMemcpy(dVn, Vn, 2 * sizeof(double) * ext.n(), hipMemcpyHostToDevice);
  }
  ...
  if (Jn) {
    hipMalloc((void**)&dJn, 4 * sizeof(double) * ext.n());
    hipMemcpy(dJn, Jn, 4 * sizeof(double) * ext.n(), hipMemcpyHostToDevice);
  }
  ...
  if (Sn) {
    hipMalloc((void**)&dSn, sizeof(double) * ext.n());
    hipMemcpy(dSn, Sn, sizeof(double) * ext.n(), hipMemcpyHostToDevice);
  }
  ...
  cp_t *dcps;
  // hipMalloc((void**)&dcps, sizeof(cp_t) * ext.n() * 2);
  hipMalloc((void**)&dcps, 1024*1024*64); // sizeof(cp_t) * ext.n() * 2);
  checkLastCudaError("[FTK-CUDA] error: sweep_simplices: hipMalloc/hipMemcpy");

  fprintf(stderr, "calling kernel func...\n");
  hipLaunchKernelGGL(HIP_KERNEL_NAME(sweep_simplices<scope>), dim3(gridSize), dim3(blockSize), 0, 0, 
      current_timestep, 
