    ...
    const double *F_n)
{
  ...
  if (F_c) {
    hipMalloc((void**)&dF_c, sizeof(double) * ext.n());
    checkLastCudaError("[FTK-CUDA] error: sweep_simplices: allocating dF_c");
    hipMemcpy(dF_c, F_c, sizeof(double) * ext.n(), hipMemcpyHostToDevice);
    checkLastCudaError("[FTK-CUDA] error: sweep_simplices: copying drho_c");
  }
  if (F_n) {
    hipMalloc((void**)&dF_n, sizeof(double) * ext.n());
    checkLastCudaError("[FTK-CUDA] error: sweep_simplices: allocating drho_l");
    hipMemcpy(dF_n, F_n, sizeof(double) * ext.n(), hipMemcpyHostToDevice);
  ...
  if (dF_c) hipFree(dF_c);
  if (dF_n) hipFree(dF_n);
  hipFree(dncps);
  hipFree(dcps);
