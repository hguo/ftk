    ...
    const double *Sn)
{
  ...
  if (Vc) {
    hipMalloc((void**)&dVc, 3 * sizeof(double) * ext.n());
    checkLastCudaError("[FTK-CUDA] error: sweep_simplices: allocating dVc");
    hipMemcpy(dVc, Vc, 3 * sizeof(double) * ext.n(), hipMemcpyHostToDevice);
    checkLastCudaError("[FTK-CUDA] error: sweep_simplices: copying dVc");
  }
  if (Vn) {
    hipMalloc((void**)&dVn, 3 * sizeof(double) * ext.n());
    checkLastCudaError("[FTK-CUDA] error: sweep_simplices: allocating dVl");
    hipMemcpy(dVn, Vn, 3 * sizeof(double) * ext.n(), hipMemcpyHostToDevice);
    checkLastCudaError("[FTK-CUDA] error: sweep_simplices: copying dVl");
  }
  
  double *dJc = NULL, *dJn = NULL;
  if (Jc) {
    hipMalloc((void**)&dJc, 9 * sizeof(double) * ext.n());
    hipMemcpy(dJc, Jc, 9 * sizeof(double) * ext.n(), hipMemcpyHostToDevice);
  }
  if (Jn) {
    hipMalloc((void**)&dJn, 9 * sizeof(double) * ext.n());
    hipMemcpy(dJn, Jn, 9 * sizeof(double) * ext.n(), hipMemcpyHostToDevice);
  }
