...
GLHeader hdr[2];
hypermesh::ndarray<float> Re[2], Im[2], Rho[2], Phi[2];
hypermesh::regular_simplex_mesh m(4);
...
std::vector<std::string> glob_filenames(const std::string& pattern)
{
...
bool load_data(const std::string& filename, 
    GLHeader &hdr,
    hypermesh::ndarray<float> &Re, 
    hypermesh::ndarray<float> &Im, 
    hypermesh::ndarray<float> &Rho, 
    hypermesh::ndarray<float> &Phi)
{
